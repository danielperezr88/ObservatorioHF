...
def AnalizeAndSave(texto, search_id, created_at, category):
    hashtags =[]
    lideres =[]
    concepts =[]
    urls=[]
#    texto = textos[search_id]
#    print(texto)
    texto = re.sub(r'[?|$|.|!|¡|¿|-|,|\\|"|(|)|”|:|“]',r'',texto)
...
#print("concepts_pos")
    ...
    try:
        ...
        registro = (search_id, created_at, concepts_json , lideres_json , hashtags_json, urls_json, category)
        print(registro)
...
for category in categories:
    for dayBack in range(1,30):
        #  ¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡  CAMBIAR EL timedelta (1)                              
        yesterday = date.today() - timedelta(dayBack)                  
        textos.clear()
        ...
        select_str = "select * from tweets where date(created_at) = '" + str(yesterday) +"' AND sentiment ='" + category +"' \
            AND search_id not in (SELECT search_id FROM `frecuents` where date(created_at) = '" + str(yesterday) +"' AND sentiment ='" + category +"')"
        #print(select_str) 
        ...
        #    print (row[8])
            ...
            search_id = str(row[1])
            if search_id not in textos.keys():
                textos[search_id] = texto_tweet
            else:
                textos[search_id] += " " + texto_tweet
            #print(texto_tweet)
        ...
        if bool(textos):
            for search_id in textos:
                AnalizeAndSave(textos[search_id], search_id, yesterday, category)